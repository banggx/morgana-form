...
import { moveComponent, selectComponent, toggleHidden, toggleLocked, changeComponentName } from '@/store/componentReducer'
...
export default function ComponentLayer() {
  ...
  const handleEditComponent = (ev: React.MouseEvent, component: ComponentInfoType) => {
    ev.stopPropagation()
  ...
  const handleUpdateComponentName = (ev: React.KeyboardEvent<HTMLInputElement>) => {
    const value = ev.currentTarget.value
    if (value.trim()) {
      dispatch(changeComponentName({
        id: editId!,
        name: value.trim()
      }))
    }
    ...
    <SortableContainer items={components} onDragEnd={handleDragEnd}>
      {
        components.map(component => {
          return <SortableItem id={component.id} key={component.id}>
            <div
              key={component.id}
              className={cn('w-full h-10 border border-gray-100 border-solid rounded-md flex items-center justify-between mb-2 px-4 shadow-sm cursor-pointer dark:border-slate-500', {
                'bg-cyan-600 text-slate-50 dark:bg-slate-500': component.id === selectedId,
            ...
            >
              {
                editId === component.id
                  ? <Input
                    ...
                    />
                  : <>
                      <div className='overflow-hidden text-ellipsis whitespace-nowrap mr-2'>{component.name}</div>
