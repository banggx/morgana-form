...
function ComponentNodeList() {
  ...
  useEffect(() => {
    setExistMap(() =>
      nodes.reduce<Record<string, boolean>>((acc, item) => {
  ...
  const setDragNodeHandle = (
    ev: React.DragEvent<HTMLDivElement>,
    id: string
  ) => {
    ev.dataTransfer.setData(
      DRAG_NODE_KEY,
      JSON.stringify({ type: GraphNodeType.Component, fromId: id })
    );
    ev.dataTransfer.effectAllowed = "move";
  };

  if (!components.length) {
    return <Empty content="暂未添加表单组件" />;
  }
    ...
    <div>
      {components.map((component) => (
        <div
          key={component.id}
          className={cn(
            "w-full h-10 border border-slate-200 broder-solid rounded-md px-2 shadow-md leading-10 mb-2 cursor-move dark:border-slate-600",
            {
              "bg-slate-300 cursor-not-allowed dark:bg-slate-600":
                existMap[component.id],
            }
          )}
...
export default function FlowNodeTree() {
  ...
  return (
    <div className="w-[280px] flex-none bg-white p-2 shadow-md overflow-auto dark:bg-slate-950">
      <Tabs
        size="small"
        type="card"
        items={FlowNodeTabs}
